    ...
    <section className="my-8">
      ...
      <div className="flex flex-wrap justify-center gap-6">
        {partners.map((partner, index) => (
          <div key={index} className="w-40 flex flex-col items-center space-y-2 hover:opacity-80 transition">
            <a
              key={index}
              href={partner.link}
              target="_blank"
            ...
            <p className="text-sm font-bold text-center">{partner.name}</p>
            <p className="text-xs font-medium text-center">{partner.description && partner.description}</p>
          </div>
        ))}
      </div>
    </section>
